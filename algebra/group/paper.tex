...
Commutativity of diagram forces us to define $\varphi([a]) = f(a)$ for any $a \in A$.
Moreover, the diagram
\input{img/08}
forces us to define $\varphi([w]) = f(w_{1}) \cdot \cdots \cdot f(w_{n})$.

When $A$ is a singleton, then we see that $F(A) \iso \dZ$, i.e., the free group on one generator is a cyclic group.
\end{document}
