...
\todo{Check this observation, although we know that every accessible category is an inductive system}

\begin{rmk}
  In a $\lambda$-accessible category, every object is a $\lambda$-filtered colimit of objects in $\Pres_{\lambda}(\iC)$ that $\lambda_{\iota}$ preserves.
  Thus, $\yon_{\iota}(c)$ is a $\lambda$-filtered colimit in $\Psh(\Pres_{\lambda}(\iC)\op)$.
\end{rmk}

\begin{defn}
  Let $\iC$ be a $\lambda$-accessible category.
  We define the category of \emph{ind-objects} $\Ind(\iC)$ as the full subcategory spanned by $\colim_{(a,f) \in (\iota \dn c)}\iC(\iota\blank, a)$ for $c \in \iC$.
\end{defn}

\begin{rmk}
  The restricted Yoneda embedding $\yon_{\iota} : \iC \to \Ind(\iC)$ is clearly an equivalence of categories.
\end{rmk}
