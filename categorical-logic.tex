...
\begin{notn}
  Let $\iC$ be a $\lambda$-accessible category, we write $\Pres_{\lambda}\iC$ for the full subcategory of $\iC$ spanned by $\lambda$-presentable objects.
\end{notn}
...
\begin{defn}
  A diagram $D : \iD \to \iC$ is finite if its scheme $\iD$ has finitely many morphisms.
\end{defn}
...
\begin{eg}
  Let $\iC$ be a small category.
  Orthogonality in $\CSet^{\iC\op}$ expresses the preservation of specified limits in $\iC$ (i.e., colimits in $\iC\op$).
...
\begin{thm}[Representation theorem]
  Let $\lambda$ be a regular cardinal.
  For each category $\iC$, the following are equivalent:
  \begin{enumerate}
  \item $\iC$ is locally $\lambda$-presentable.
  \item $\iC$ is equivalent to $\Cont_{\lambda}\iA$ for some small category $\iA$.
  \item $\iC$ is equivalent to a $\lambda$-orthogonality class in $\CSet^{\iA}$ for some small category $\iA$.
  \item $\iC$ is equivalent to a full and reflective subcategory of $\CSet^{\iA}$ closed under $\lambda$-directed colimits for some small category $\iA$.
  \item $\iC$ is a $\lambda$-free cocompletion of a small category $\iA$.
  \end{enumerate}
\end{thm}
