...
\end{lem}
\begin{proof}
  Let $r : c \to c'$ be a retraction with section $s : c' \to c$.
...
\begin{defn}
  An algebraic theory is said to be \emph{canonical} if it is idempotent complete.
\end{defn}

\begin{defn}
  Let $\iC$ be a category, define a category $\Idem(\iC)$ as follows:
  \begin{itemize}
  \item[] Objects: $(c \in \iC, f : c \to c)$, where $f$ is an idempotent in $\iC$.
  \end{itemize}
\end{defn}

\begin{lem}
  Let $\cT$ be an algebraic theory.
  The Yoneda embedding $\yon : \cT \to \Alg(\cT\op)_{pp}$ is an idempotent completion of $\cT$, where $\Alg(\cT\op)_{pp}$ is the full subcategory of $\Alg(\cT\op)$ spanned by perfectly presentable algebras.
\end{lem}
\begin{proof}
  By \cref{thm:alg-pp-ret}, $\Alg(\cT\op)_{pp}$ consists of retracts of representable algebras.
  \todo{}
\end{proof}
...
\begin{proof}
  ...
  Since $\cT_{2}$ is Cauchy complete, $F(\cT_{1}(t,\blank)) \iso \cT_{2}(t', \blank)$ for some $t' \in \cT_{2}$.
  Thus, we define $M : t \mapsto t'$.
  The Yoneda lemma defines an action on morphisms as follows
  % https://q.uiver.app/#q=
  \[\begin{tikzcd}
      {F(\cT_{1}(t_{1},\blank))} & \iso & {\cT_{2}(t_{1}',\blank)} \\
      \\
      {F(\cT_{1}(t_{2},\blank))} & \iso & {\cT_{2}(t_{2}',\blank)}
      \arrow["{F(f\circ \blank)}"', from=1-1, to=3-1]
      \arrow[dashed, from=1-3, to=3-3]
    \end{tikzcd}\]
  Note that a natural transformation $\cT_{2}(t_{1}',\blank) \to \cT_{2}(t_{2}',\blank)$ corresponds to a morphism $t_{2}' \to t_{1}'$.
  Thus, we have a functor $M : \cT_{1}\op \to \cT_{2}\op$.
\end{proof}
