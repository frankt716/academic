...
\begin{lem}\label{lem:colimits-strong-generators}
  ...
  If $\iG$ is a dense set of $\iC$, i.e., every object $c \in \iC$ is the colimit of the domain functor $\dom : \iG \dn c \to \iC$, then $\iG$ is a strong generator.
\end{lem}
\begin{proof}
  As $c$ is the colimit of $\dom$, $c$ is a regular quotient of $\coprod_{\iG \dn c}G$.
  That is,
  \[
...
\begin{proof}
  ...
  Let $\iota : \iG \into \iC$ be the inclusion functor and $\yon_{\iG} : \iC \to \CSet^{\iG\op}$ be the restricted Yoneda embedding along $\iota$.
  This is right adjoint to the left Kan extension $\iota^{*}$.
  Composing $\yon_{\iG}$ with the localizer in \cref{cor:alg-reflective} yields $y^{*}\yon_{\iG} : \iC \to \Alg(\iG\op)$.
  We show that this functor is an equivalence.
  % https://q.uiver.app/#q=
  \[\begin{tikzcd}
      & \iG && \iC \\
      {\Alg(\iG\op)} && {\CSet^{\iG\op}}
      \arrow["\iota", hook, from=1-2, to=1-4]
      \arrow["\yon"{description}, from=1-2, to=2-3]
      \arrow["{\yon_{\iG}}", from=1-4, to=2-3]
      \arrow[""{name=0, anchor=center, inner sep=0}, "y"', from=1-2, to=2-1]
      \arrow["{y^{*}}", from=2-3, to=2-1]
      \arrow["{\id_{y}}"', shorten <=9pt, Rightarrow, from=0, to=2-3]
    \end{tikzcd}\]
  First, we show that $y^{*}\yon_{\iG}$ is essentially surjective on objects.
  Clearly, $\id_{\Alg(\iG\op)}$ is the essentially unique extension along $y$ in \cref{thm:alg-free-cocompletion}.
  As we have $y^{*}\yon_{\iG}\iota^{*}y = y^{*}\yon_{\iG}\iota = y$, if we can show that $y^{*}\yon_{\iG}\iota^{*}$ preserves sifted colimit, then $y^{*}\yon_{\iG}\iota^{*} \iso \id$.
  Since $y^{*}$ and $\iota^{*}$ are left adjoints, it suffices to show that $\yon_{\iG}$ preserves sifted colimits.
  As $\iG$ consists of perfectly presentable objects and colimits are computed pointwise, we have
  \[
    \yon_{\iG}(\colim D)(G) = \iC(\iota G, \colim D) \iso \colim\iC(\iota G,D\blank) \iso (\colim\yon_{\iG}D)(G)
  \]
  Now, we show that $y^{*}\yon_{\iG}$ is fully faithful.
  For any $c \in \iC$, we have
  \[
    y^{*}\yon_{\iG}c = y^{*}\iC(\iota\blank, c) \iso \iC(\iota\blank, c)
  \]
  as $y^{*}$ is a localizer.
  Indeed, the functor $\iC(\iota\blank, c)$ is an algebra.
  Thus, it suffices to show how to lift each homomorphism $\alpha : \iC(\iota\blank, c) \to \iC(\iota\blank, c')$ to a unique morphism $a : c \to c'$.
  Let $\chat := \coprod_{(G,g) \in \iG \dn c}G$ and $\ctil := \colim_{(G,g) \in \iG \dn c}G$
  As $\iG$ is dense, every object $c \in \iC$ can be expressed as $\ctil$.
  In particular, $\chat \in \iG$ and $\ctil$ is a sifted colimit because $\iG$ is closed under coproducts.
  Consider $\alpha_{\chat}(\ev_{c}) : \chat \to c'$.
  The whisker $\alpha(\ev_{c}) \cdot \beta$, where $\beta$ is the coproduct cocone, yields a cocone under $\dom$ with nadir $\ctil'$.
  % https://q.uiver.app/#q=
  \[\begin{tikzcd}
      && \chat \\
      \dom && \ctil && {\ctil'}
      \arrow[Rightarrow, from=2-1, to=2-3]
      \arrow["\beta", Rightarrow, from=2-1, to=1-3]
      \arrow["{\alpha(\ev_{c})}", from=1-3, to=2-5]
      \arrow[dashed, from=2-3, to=2-5]
    \end{tikzcd}\]
  Thus, there is a unique morphism $\atil : \ctil \to \ctil'$ that determines the required morphism $a : c \to c'$ whose image under $\iota^{*}\yon_{\iG}$ is essentially $\alpha$.
\end{proof}
