...
\end{node}
\begin{node}
  This is a functor $F_{a} : \dA \to \Alg(A)$ whose composition with the full inclusion functor $\iota : \Alg(A) \into \CSet^{\dA}$ is the Yoneda embedding.
  \[
...
\end{node}
\begin{node}
  Since $\yon_{a}$ and $\iota$ are fully faithful, so is $T_{a}$.
\end{node}
