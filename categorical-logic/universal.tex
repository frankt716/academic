...
\begin{prop}
  ...
  The morphism $1 : A \to A^{0}$ is always epimorphic, and if there exists $g : A^{0} \to A$ then $1$ is a retraction.
\end{prop}
\begin{proof}
  \begin{node}
    Since $A^{0}$ is a terminal object in $\dA$, $1 \circ g = \id_{A^{0}}$ for any $g : A^{0} \to A$.
    Thus, $1$ is a retraction whenever $g$ exists.
  \end{node}
  \begin{node}
    If $g$ does not exist, then $1$ is trivially epimorphic.
  \end{node}
\end{proof}

\begin{eg}
  The identity functor $\dN\op \to \dN\op$ is a Lawvere theory, which we call ``the theory of equality''.
  This is the initial object of $\CLaw$.
\end{eg}

\begin{node}
  Given a family of Lawvere theories indexed by a small set $\Lambda$ (i.e., a functor $\dF : \Lambda \to \CLaw$), we can construct the coproduct of Lawvere theories in this family.
  The category $\dF\Lambda$ is defined as follows:
  \begin{enumerate}
  \item The objects are the natural numbers.
  \item If $f : A^{n} \to A$ is an $n$-ary operation of some Lawvere theory in the family $\dF$, then $\fhat : n \to 1$ is a morphism of $\dF\Lambda$.
  \item If $t_{1},\ldots,t_{n} : m \to 1$ are morphisms of $\dF\Lambda$, then $(t_{1},\ldots,t_{n}) : m \to n$ is a morphism of $\dF\Lambda$.
  \item If $t : j \to k$ and $t' : i \to j$ are morphisms of $\dF\Lambda$, then $tt' : i \to k$ is a morphism of $\dF\Lambda$.
  \end{enumerate}
\end{node}
...
\begin{node}
  ...
  $T$ maps each Lawvere theory $A$ to a sequence of sets whose $n$-th element is the set $\Hom(A^{n}, A)$, i.e., the set of all $n$-ary operations.
