...
\begin{defn}\label{defn:lawvere-theory-algebraic-theory}%
  A \emph{Lawvere theory} is a category $\cT$ with finite products equipped with a strictly finite-power preserving functor $p : \dF\op \to \cT$, where $\dF$ is the skeletal category of $\CFinSet$.
...
\begin{node}%
  Let $(\cT,p)$ be a Lawvere theory, we write $T^{n}$ for $p(n)$.
  For each morphism $n \to 1 \in \dF\op$, the functor $p$ picks out an $n$-ary operation $T^{n} \to T$.
\end{node}
