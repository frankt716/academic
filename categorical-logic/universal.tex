...
\label{sec:regular-categories-equivalence-relations}

\begin{rmk}
  \label{rmk:relations-motivation}
...
\begin{defn}
  ...
  A \emph{relation} on an object $A$ of a category $\iC$ is an object $R \in \iC$ with a pair of monomorphisms $r_{1},r_{2} : R \toto A$.
  For each object $X \in \iC$ we write
  \[
    R_{X} = \{(r_{1}x, r_{2}x) \mid x \in \iC(X,R)\}
  \]
  We use this notation as a shorthand for the following:
  \[
    R_{X} = \{(y, z) \mid \exists x \in \iC(X,R).\,y = r_{1}x \land z = r_{2}x\}
  \]
...
\begin{prop}
  ...
  \item a morphism $\tau : R \times_{A} R \to R$ such that $r_{1}\tau = \rho_{1}$ and $r_{2}\tau = \rho_{2}$, where $\rho_{1}$ and $\rho_{2}$ are the pullback of $r_{1}$ and $r_{2}$.
  \end{enumerate}
\end{prop}
\begin{proof}
  \begin{enumerate}
  \item[]
  \item Suppose that $(R,r_{1},r_{2})$ is an equivalence relation on $A$.
    Then the relation $R_{A} = \{(r_{1}x,r_{2}x) \mid x \in \iC(A,R)\}$ on $\iC(A,A)$ is reflexive.
    In particular $(\id_{A},\id_{A}) \in R_{A}$.
    Thus, there is a morphism $\delta : A \to R$ such that $r_{1}\delta = \id_{A} = r_{2}\delta$.
