...
\begin{defn}
  \label{defn:lawvere-theory-algebraic-theory}
  An \emph{algebraic theory} $\cT$ is a category with a denumerable set of objects
  \[
    \Set{T^{0}, T, T^{2}, T^{3}, \ldots}
  \]
...
\begin{node}
  \label{node:lawvere-theory-canonical-presentation}
  We take each element of $\cT(T^{n}, T)$ as an $n$-ary function symbol of $\dT$.
  For each term $t$ in variables $\Set{x_{1},\ldots,x_{n}}$, we associate $t$ with a morphism $\tau \in \cT(T^{n}, T)$ inductively as follows:
  \begin{enumerate}
