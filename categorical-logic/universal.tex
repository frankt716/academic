...
\begin{defn}
  ...
  \[
    \colim_{\iC}\prod_{\iJ}D(c,j) \to \prod_{\iJ}\colim_{\iC}D(c,j)
  \]
  is an isomorphism.
...
\begin{lem}
  Let $F : \iD \to \iE$ be a functor.
  $F$ is final if and only if for each $d \in \iD$, the slice category $d \dn F$ is connected.
\end{lem}

\begin{thm}
  An inhabited small category $\iD$ is sifted if and only if for each $(d, d') \in \iD \times \iD$, the category of cospans $(d, d') \dn \Delta$ is connected.
\end{thm}
\begin{proof}
  \begin{node}
    Given any two diagrams $F, F' : \iD \to \CSet$, consider the diagram $F \times F' : \iD \times \iD \to \CSet$ defined by
    \[
      F \times F' : (d, d') \mapsto Fd \times F'd'
    \]
    For any set $X$, the functor $X \times \blank$ is left adjoint to $\CSet(X, \blank)$.
    Thus, $X \times \blank$ preserves colimits.
    The colimit of this diagram can be computed as follows
    \[
      \colim_{(d, d') \in \iD \times \iD}(Fd \times F'd') \iso \colim_{d \in \iD}(\colim_{d' \in \iD}(Fd \times F'd')) \iso \colim_{d \in \iD}Fd \times \colim_{d \in \iD}Fd
    \]
    Now, consider the following diagram
    \[
      % https://q.uiver.app/#q=
      \begin{tikzcd}
	{\colim_{\iD}(Fd \times F'd)} && {\colim_{\iD}Fd \times \colim_{\iD}F'd} \\
	\\
	{\colim_{\iD}(F \times F')\Delta} && {\colim_{\iD \times \iD}(F \times F')}
	\arrow["\gamma"', from=3-1, to=3-3]
	\arrow["\iso"', from=3-3, to=1-3]
	\arrow[Rightarrow, no head, from=1-1, to=3-1]
	\arrow["\delta", from=1-1, to=1-3]
      \end{tikzcd}
    \]
    Note that the diagram commutes as the canonical map out of a colimit is unique.
  \end{node}
  \begin{node}
    If $\iD$ is sifted, then $\delta$ is an isomorphism.
    Thus, $\gamma$ is an isomorphism.
    Conversely, if $\Delta$ is final then $\gamma$ is an isomorphism.
    Thus, $\delta$ is an isomorphism.
  \end{node}
\end{proof}
\todo{Investigate remark 2.2.2}

\section{Reflexive coequalizers}
\label{sec:reflexive-coequalizers}

\begin{defn}
  Let $\iJ$ be the following sifted category
  \[
...
\begin{node}
  Since reflexive coequalizers are sifted colimits, any algebraic category has reflexive coequalizers by \cref{thm:algebraic-categories-has-sifted-colimits}.
\end{node}
...
\begin{proof}
  \begin{node}
    We have already shown that algebraic categories have reflexive coequalizers and filtered coequalizers.
    It suffices to construct binary coproducts.
  \end{node}
  \begin{node}
    Let $F$ and $G$ be algebras expressed as $\colim\yon\Pi_{F}$ and $\colim\yon\Pi_{G}$.
    Construct the coproduct $\colim\yon\Pi_{F} + \colim\yon\Pi_{G}$.
    Since colimits commute with coproducts, this is naturally isomorphic to $\colim(\yon\Pi_{F} + \yon\Pi_{G})$.
    The indexing category of the diagram $\yon\Pi_{F} + \yon\Pi_{G}$ is sifted, so its colimit commutes with finite products.
  \end{node}
\end{proof}
