...
\todo{characterize sifted categories.}
...
\begin{thm}
  ...
  Every functor $F : \iC \to \CSet$ is the colimit of representable functors.
  That is, $F \iso \colim \yon\Pi$, where $\Pi : \int F \to \iC$ is the projection functor.
\end{thm}
...
\begin{node}
  The algebra $\dA(A^{n},\blank)$ is precisely the free algebra on $n$ generators.
\end{node}

\begin{node}
  This determines a functor $F_{a} : \dA \to \Alg(A)$ whose composition with the full inclusion functor $\iota : \Alg(A) \into \CSet^{\dA}$ is the Yoneda embedding.
  \[
    % https://q.uiver.app/#q=
    \begin{tikzcd}
      {\Alg(A)} \\
      \\
      \dA\op && {\CSet^{\dA}}
      \arrow["{\yon_{a}}"', hook, from=3-1, to=3-3]
      \arrow["\iota", hook, from=1-1, to=3-3]
      \arrow["{F_{a}}", from=3-1, to=1-1]
    \end{tikzcd}
  \]
\end{node}
\begin{node}
  Since $\yon_{a}$ and $\iota$ are fully faithful, so is $F_{a}$.
\end{node}
