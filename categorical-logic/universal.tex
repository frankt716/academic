...
\begin{defn}
  A functor $F : \iC \to \iD$ is \emph{final} if, for every diagram $D : \iD \to \iE$ such that $\colim(DF)$ exists, then $\colim D$ exists and the canonical morphism $\colim(DF) \to \colim D$ is an isomorphism.
\end{defn}

\begin{prop}
  Let $F : \iC \to \iD$ be a functor.
  The following conditions are equivalent:
  \begin{enumerate}
  \item $F$ is final;
  \item $F$ satisfies the finality condition with respect to all representable functors $\iD(d, \blank)$;
  \item For every object $d$ of $\iD$, the slice category $d \dn F$ is connected.
  \end{enumerate}
\end{prop}
\begin{proof}
  \begin{node}
  ...
  \begin{node}
    Assume (2).
    Since the diagram $\iD(d, \blank) : \iD \to \CSet$ has colimit $1$, $1$ is also a colimit of $\iD(d, F\blank)$.
    Thus, the slice category $d \dn F$ has an initial object and, therefore, connected.
  \end{node}
  \begin{node}
    Assume (3).
    Let $\gamma : e \to DF$ be a colimit cone of $DF$.
    We show that $e$ is a colimit of the diagram $D$.
    Pick an arbitrary $c \in \iC$.
    For each $d \in D$, pick an arbitrary morphism $u_{d} : d \to Fc$.
  \end{node}
\end{proof}
