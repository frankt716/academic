...
≡-elim : {ℓ : Level} {A : Type ℓ} {B : {x y : A} → x ≡ y → Type ℓ}
       → ({x : A} → B (refl x))
       → {x y : A} → (p : x ≡ y) → B p
≡-elim f (refl _) = f
...
≡-refl-fun : {ℓ : Level} {A : Type ℓ} {x : A} → x ≡ x
≡-refl-fun = refl _

≡-sym-fun : {ℓ : Level} {A : Type ℓ} {x y : A} → x ≡ y → y ≡ x
≡-sym-fun {ℓ} {A} {x} {y} p = ≡-elim {ℓ} {A} {λ {x'} {y'} p' → y' ≡ x'} (refl _) p
...
Pi : {l1 l2 : Level} (A : Type l1) (B : A → Type l2) → Type (l1 ⊔ l2)
Pi A B = (x : A) → B x
